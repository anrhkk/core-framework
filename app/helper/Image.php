...
class Image
{
     ...
     * @return bool|string
     */
    public static function thumb($img, $outFile, $thumbWidth, $thumbHeight, $thumbType)
    {
        if (!self::check($img)) {
            return false;
        }
        ...
        //获得相关尺寸
        $thumb_size = self::thumbSize($imgWidth, $imgHeight, $thumbWidth, $thumbHeight, $thumbType);
        //原始图像资源
     ...
     * @return bool
     */
    public static function water($img, $outImg)
    {
        //验证原图像
        if (!self::check($img))
            return false;
        ...
        $waterImgOn = self::check($waterImg) ? 1 : 0;
        ...
        if ($waterImgOn) {
            ...
            if (empty($text) || strlen(self::waterTextColor) != 7)
                return false;
            $textInfo = imagettfbbox(self::waterTextSize, 0, self::waterTextFont, $text);
