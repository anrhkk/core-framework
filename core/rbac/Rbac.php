...
class Rbac
{
    ...
    {
        $this->user_table = config('database.prefix') . config('rbac.user_table');
        $this->role_table = config('database.prefix') . config('rbac.role_table');
        $this->node_table = config('database.prefix') . config('rbac.node_table');
        $this->user_role_table = config('database.prefix') . config('rbac.user_role_table');
        $this->access_table = config('database.prefix') . config('rbac.access_table');
        $this->auth_key = config('rbac.auth_key');
    }
...
//        dump($_SESSION);
    ...
    {
        //检查不需要验证的动作
        foreach (config('rbac.no_auth') as $noAuth) {
        ...
        //没有权限节点或时时验证时
        //都要从数据库中取权限节点信息
        if (!isset($_SESSION['__RBAC__']) || config('rbac.type') == 1) {
    ...
    public function isSuperUser()
    {
        ...
        $username = Db::table(config('rbac.user_table'))->where($this->auth_key, $_SESSION[$this->auth_key])->getFieldValue('username');
        if ($username == config('rbac.super_user')) {
