...
class Weixin extends Error
{
    ...
    public function getAccessToken($cacheKey = '')
    {
        ...
        if ($token = Cache::dir('data/weixin')->get($cacheKey)) {
        ...
        if (array_key_exists('errcode', $json) && $json['errcode'] != 0) {
            ...
            //应该保存缓存。。。
            Cache::dir('data/weixin')->set($cacheKey, $this->access_token, 7000);
    ...
    public function instance($type)
    {
        switch ($type) {
            case 'message':
                //消息管理
                return new \core\Weixin\Message;
            case 'qrcode':
                //二维码
                return new \core\Weixin\Qrcode;
            case 'customservice':
                //客服
                return new \core\Weixin\CustomService;
            case 'button':
                //自定义菜单
                return new \core\Weixin\Button;
            case 'material':
                //素材管理
                return new \core\Weixin\Material;
            case 'group':
                //会员组管理
                return new \core\Weixin\Group;
            case 'user':
                //会员管理
                return new \core\Weixin\User;
        }
    }
