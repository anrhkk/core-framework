...
class DB
{
    ...
    public function __construct()
    {
        ...
        try {
            ...
            $dsn = $type . ':host=' . $this->server . (isset($port) ? ';port=' . $port : '') . ';dbname=' . $this->database;
            // Make MySQL using standard quoted identifier
            $commands[] = 'SET SQL_MODE=ANSI_QUOTES';

            if (in_array($type, explode(' ', 'mariadb mysql')) && $this->charset) {
    ...
    {
        return '"' . str_replace('.', '"."', preg_replace('/(^#|\(([a-z0-9]+)\)\s*)/', '', $string)) . '"';
    }
    ...
    protected function inner_join($data, $join, $outer_join)
    {
        $haystack = [];

        foreach ($data as $value) {
            $haystack[] = '(' . $this->data_implode($value, $join) . ')';
        }

        return implode($outer_join . ' ', $haystack);
    }

    protected function function_quote($column, $string)
    {
        return (strpos($column, '#') === 0 && preg_match('/^[A-Z0-9\_]*\([^)]*\)$/', $string)) ? $string : $this->quote($string);
    }

    protected function data_implode($data, $join, $outer_join = null)
    {
        $wheres = [];
        ...
        foreach ($data as $key => $value) {
            ...
            if (preg_match("/^(AND|OR)(\s+#.*)?$/i", $key, $relation_match) && $type == 'array') {
                $wheres[] = 0 !== count(array_diff_key($value, array_keys(array_keys($value)))) ?
                    ...
                    '(' . $this->inner_join($value, ' ' . $relation_match[1], $join) . ')';
            } else {
                ...
                if (isset($match[4])) {
                    ...
                    if ($operator == '!') {
                        switch ($type) {
                            ...
                            case 'string':
                                $wheres[] = $column . ' != ' . $this->function_quote($key, $value);
                                break;
                        }
                    }
                    ...
                    if ($operator == '~' || $operator == '!~') {
                        ...
                        foreach ($value as $item) {
                            ...
                            $like_clauses[] = $column . ($operator === '!~' ? ' NOT' : '') . ' LIKE ' . $this->function_quote($key, $item);
                        }
                    ...
                    if (in_array($operator, ['>', '>=', '<', '<='])) {
                        if (is_numeric($value)) {
                            ...
                            $wheres[] = $column . ' ' . $operator . ' ' . $this->function_quote($key, $value);
                        } else {
    ...
    protected function select_parse($table, $join, &$columns = null, $where = null, $column_fn = null)
    {
    ...
    {
        $query = $this->query($this->select_parse($table, $join, $columns, $where));
        ...
        foreach ($datas as $data) {
            ...
            foreach ($data as $key => $value) {
                ...
                switch (gettype($value)) {
                    ...
                    case 'array':
                        preg_match("/\(([a-z0-9]+)\)\s*([\w]+)/i", $key, $column_match);
                        $values[] = isset($column_match[1]) ? $this->quote($column_match[1]($value)) : $this->quote(arr2str($value));
                        break;
                    ...
                    case 'double':
                    case 'string':
                        $values[] = $this->function_quote($key, $value);
                        break;
                }
            }
            $this->execute('INSERT INTO "' . $this->prefix . $table . '" (' . implode(', ', $columns) . ') VALUES (' . implode($values, ', ') . ')');
        ...
        foreach ($data as $key => $value) {
            ...
            if (isset($match[3])) {
                ...
                switch (gettype($value)) {
                    ...
                    case 'array':
                        preg_match("/\(([a-z0-9]+)\)\s*([\w]+)/i", $key, $column_match);
                        $fields[] = $column . ' = ' . $this->quote(isset($column_match[1]) ? $column_match[1]($value) : arr2str($value));
                        break;
                    ...
                    case 'double':
                    case 'string':
                        $fields[] = $column . ' = ' . $this->function_quote($key, $value);
                        break;
                }
            }
        }
        return $this->execute('UPDATE "' . $this->prefix . $table . '" SET ' . implode(', ', $fields) . $this->where_clause($where));
    }

    public function delete($table, $where)
    {
        return $this->execute('DELETE FROM "' . $this->prefix . $table . '"' . $this->where_clause($where));
    }
    ...
    public function replace($table, $columns, $search = null, $replace = null, $where = null)
    {
        ...
        return $this->execute('UPDATE "' . $this->prefix . $table . '" SET ' . $replace_query . $this->where_clause($where));
    }

    public function get($table, $join = null, $column = null, $where = null)
    {
        $query = $this->query($this->select_parse($table, $join, $column, $where) . ' LIMIT 1');
    ...
    public function has($table, $join, $where = null)
    {
        ...
        $query = $this->query('SELECT EXISTS(' . $this->select_parse($table, $join, $column, $where, 1) . ')');
    ...
    {
        $query = $this->query($this->select_parse($table, $join, $column, $where, 'COUNT'));
    ...
    {
        $query = $this->query($this->select_parse($table, $join, $column, $where, 'MAX'));
    ...
    {
        $query = $this->query($this->select_parse($table, $join, $column, $where, 'MIN'));
    ...
    {
        $query = $this->query($this->select_parse($table, $join, $column, $where, 'AVG'));
    ...
    {
        $query = $this->query($this->select_parse($table, $join, $column, $where, 'SUM'));
     ...
     * @access public
     */
    public function getFields($table, $name = true)
    {
    ...
    public function page($table, $join, $columns = null, $where = null)
    {
        $page = Page::show($this->count($table, $where));
        $where['LIMIT'] = $page['limit'];
        return ['list' => $this->select($table, $join, $columns, $where), 'page' => $page['page']];
    }
